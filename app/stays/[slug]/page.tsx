...
export function generateStaticParams() {
  return getSeoPagesByGroup("stays").map((page) => ({ slug: page.slug }))
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const page = getSeoPage("stays", (await params).slug)
  if (!page) return { title: "Stay Type Not Found" }
  return landingPageMetadata(page, await getCatalog())
}

export default async function StayTypePage({ params }: { params: Promise<{ slug: string }> }) {
  const page = getSeoPage("stays", (await params).slug)
  if (!page) notFound()
